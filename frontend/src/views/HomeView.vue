<template>
  <div class="space-y-8">
    ...
    <div class="flex flex-col sm:flex-row gap-4">
      <div class="flex-1 ">
        <label for="kes-filter" class="block text-sm font-medium text-[color:var(--color-muted-foreground)] mb-1">Фильтр по коду КЭС</label>
        <select id="kes-filter" v-model="selectedKes" @change="handleFilterChange" class="input-field max-w-sm">
          <option value="">Все коды КЭС</option>
          <option v-for="kes in kesCodes" :key="kes" :value="kes.split(' ')[0]">{{ kes }}</option>
        </select>
      </div>
      <div>
        <label class="block text-sm font-medium text-[color:var(--color-muted-foreground)] mb-1">Фильтр по части</label>
    ...
    <div v-if="loading" class="text-center text-[color:var(--color-muted-foreground)] py-12">
      Загрузка заданий...
    </div>
    <div v-else-if="error" class="text-center text-[color:var(--color-destructive)] py-12">
      Ошибка загрузки заданий: {{ error.message }}
      <p class="text-[color:var(--color-muted-foreground)] mt-2">Убедитесь, что бэкенд запущен и задания загружены в базу данных.</p>
    </div>
    <div v-else-if="tasks.length === 0" class="text-center py-12">
        <div class="mx-auto w-24 h-24 rounded-full bg-[color:var(--color-muted)]/20 flex items-center justify-center mb-4">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 text-[color:var(--color-muted-foreground)]/50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
            </svg>
        </div>
        <h3 class="text-xl font-medium text-[color:var(--color-foreground)] mb-1">Задания не найдены</h3>
        <p class="text-[color:var(--color-muted-foreground)] mb-4">Задания, соответствующие текущим фильтрам, не найдены. Попробуйте изменить настройки фильтра.</p>
    </div>
    ...
    <div v-if="totalPages > 1" class="flex justify-center items-center gap-4 mt-8">
      ...
      <span class="text-[color:var(--color-muted-foreground)]">Страница {{ currentPage }} из {{ totalPages }}</span>
