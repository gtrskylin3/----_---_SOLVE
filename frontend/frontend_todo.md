# Frontend Refactoring and Feature Implementation Plan

## Part 1: Style Refactoring and Light Theme Implementation

- [x] **Step 1: Centralize colors in `tailwind.config.js` using CSS variables.**
  - [x] Modify `tailwind.config.js` to use HSL CSS variables for the color palette.
- [x] **Step 2: Define theme variables in `src/style.css`.**
  - [x] Remove the non-standard `@theme` block.
  - [x] Define `:root` (dark theme) and `.light` (light theme) color variables using HSL values.
- [x] **Step 3: Refactor custom classes in `style.css`.**
  - [x] Replace hardcoded colors in classes like `.task-card`, `.btn-primary`, etc., with semantic Tailwind colors (e.g., `bg-card`).
- [x] **Step 4: Create a theme switcher.**
  - [x] Add a theme toggle button to `DefaultLayout.vue`.
  - [x] Implement the logic to switch the theme and persist the choice in `localStorage`.

## Part 2: Mobile Responsiveness

- [x] **Step 1: Adapt navigation.**
  - [x] Hide the default navigation on mobile screens.
  - [x] Add a "burger" menu button.
  - [x] Create a mobile-friendly menu (dropdown or sidebar).
- [x] **Step 2: Adapt main content.**
  - [x] Analyze and refactor pages (`DashboardView`, `HomeView`, etc.) for responsiveness.
  - [x] Use Tailwind's responsive utilities to adjust layouts, grids, and typography.
  - [x] Ensure components like `TaskCard.vue` are responsive.
